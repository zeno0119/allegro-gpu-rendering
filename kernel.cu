...
__global__ void step(int *src, int *dst, int width, int height) {
    ...
    if (width <= idx || height <= idy)
        return;
    int neighbor = 0;
    for (int i = -1; i < 2; i++) {
        for (int j = -1; j < 2; j++) {
            neighbor += src[(idx + i + width) % width +
                            ((idy + j + height) % height) * width] >= 1
                            ? 1
                            : 0;
        }
    }
    if (src[idx + width * idy] >= 1) {
        dst[idx + width * idy] =
            neighbor == 3 || neighbor == 4 ? src[idx + width * idy] + 1 : 0;
    } else {
        dst[idx + width * idy] = neighbor == 3 ? 1 : 0;
    }
}

__global__ void render(unsigned int *dst, int *data, int width, int height,
                       int pitch, int size) {
    //画素単位でスレッドを立ててレンダリング
    const int idx = threadIdx.x + blockDim.x * blockIdx.x;
    const int idy = threadIdx.y + blockDim.y * blockIdx.y;
    ...
    if (data[idx / size + idy / size * width] == 0)
        return;
...
int game(unsigned int *data, int state, int *init, int width, int height,
         int pitch) {
    ...
    const dim3 grid(width / 1024, height);
    ...
    if (init != nullptr) {
        cudaMemcpy(buf1, init, sizeof(int) * width * height,
    ...
    if (data != nullptr) {
        cudaMemcpy(data, buf3, sizeof(unsigned int) * width * height,
